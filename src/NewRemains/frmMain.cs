...
namespace NewRemains
{
    public partial class frmMain : Form
    {
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            this.Text = ConnectionSettings.ProgramName + " - " + Nwuram.Framework.Settings.User.UserSettings.User.Status + " - " + ", " + Nwuram.Framework.Settings.User.UserSettings.User.FullUsername;
        ...
        private void dgvMain_RowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
        {
            ...
            if (Config.statusUser.ToUpper() == "БГЛ")
            {
                if (Convert.ToInt32(dgvMain.Rows[e.RowIndex].Cells["Sgn"].Value.ToString()) == 1 || //1
        ...
        private void bwSave_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            DataTable dtIdInvVVO = new DataTable();
            ...
            if (id_otdel == 0)
            {
                ...
                cbDeps.Invoke((Action)(() => dtIdInvVVO = readSQL.getTostResult(dtpDate.Value.Date.AddDays(-1), 6)));


                if (dtIdInv == null)
                {
                    MessageBox.Show("Нет данных по инвентаризации");
                    e.Cancel = true;
                    return;
                }

                if (dtIdInvVVO == null) {
                    MessageBox.Show("Нет данных по инвентаризации на ВВО"); e.Cancel = true; return; }


                if (dtIdInvVVO.Rows[0]["idInv"] == DBNull.Value)
                {
                    MessageBox.Show("Нет данных по инвентаризации на ВВО"); e.Cancel = true; return;
                }
                ...
                formWait.Invoke((MethodInvoker)delegate
                {
                    formWait.TextWait = "Начало сохранения данных";
                    formWait.Visible = true;
                });

                readSQL.deleteConfigList(int.Parse(dtIdInv.Rows[0]["idInv"].ToString()), 1);
                readSQL.deleteConfigList(int.Parse(dtIdInvVVO.Rows[0]["idInv"].ToString()), 6);
                //cbDeps.Invoke((Action)(() => dtIdInvVVO = readSQL.getTostResult(dtpDate.Value.Date.AddDays(-1), 6)));
                ...
                //for (int i = 1; i < 50; i++)
                {
                    ...
                    if (rowSave.Count() > 0)
                    {
                        ...
                        for (int i = 0; i < rowSave.Count(); i++)
                        {
                            ...
                            //getExpDate(date, int.Parse(rowSave[i]["id_tovar"].ToString()), (decimal)rowSave[i]["netto"]);
                            //
                        }
                    }
                ...
                //Тут группировка по товарам и сумма по нетто

                var query3 = (from a in dtGlobal.AsEnumerable()
                              group a by new
                              {
                                  id_tovar = a.Field<int>("id_tovar")
                              }
                                   into g
                              select new
                              {
                                  id_tovar = g.Key.id_tovar,
                                  sumResult = g.Sum(x => x.Field<decimal>("netto")),                               
                              }
                               ).ToList();
                ...
                formWait.Invoke((MethodInvoker)delegate
                { formWait.TextWait = "Формирую сроки годности"; });
                readSQL.SetOstTovarExpDate(0, int.Parse(dtIdInv.Rows[0]["idInv"].ToString()), null, 0, true);
                foreach (var l in query3)
                {
                    //Расчёт сроков годности
                    getExpDate(date, l.id_tovar, l.sumResult);
                    //
                }
        ...
        private void getExpDate(DateTime dateIn, int id_tovar,decimal netto)
        {          
            DataTable dtIn = readSQL.GetOstTovarExpDate(dateIn.AddDays(1), id_tovar);
            if (dtIn != null)
            {
                object expData = null;
                object tmpExpData = null;
                int id_ttost = 0;
                try
                {
                    expData = dtIn.Select("netto is null")[0]["ExpiryDate"];
                    id_ttost = (int)dtIn.Select("netto is null")[0]["id_ttos"];
                }
                catch { }
                
                DataRow[] rowSort = dtIn.Select("netto is not null", "ExpiryDate DESC");
                foreach (DataRow r in rowSort)
                {
                    decimal tmpNetto = (decimal)r["netto"];
                    tmpExpData = r["ExpiryDate"];
                    //if (tmpExpData == DBNull.Value)
                    //    tmpExpData = expData;

                    if (tmpNetto > netto)
                    {
                        //Запись с netto   
                        if (tmpExpData != null && tmpExpData != DBNull.Value)
                        {
                            DataTable dtResult = readSQL.SetOstTovarExpDate(id_tovar, id_ttost, tmpExpData, netto, false);
                        }
                        netto = 0;
                    }
                    else
                    {
                        netto = netto - tmpNetto;
                        if (tmpExpData != null && tmpExpData != DBNull.Value)
                        {
                            DataTable dtResult = readSQL.SetOstTovarExpDate(id_tovar, id_ttost, tmpExpData, tmpNetto, false);
                        }
                        //Запись tmpNetto
                    }
                ...
                //if (netto > 0)
                //{
                //    if (expData != null && expData!=DBNull.Value)
                //    {
                //        DataTable dtResult = readSQL.SetOstTovarExpDate(id_tovar, id_ttost, tmpExpData, netto, false);
                //    }
                //}
            ...
            {
                //запись netto c ExpiryDate = null
                //DataTable dtResult = readSQL.SetOstTovarExpDate(id_tovar, id_ttost, tmpExpData, netto);
            }
        }
        ...
        private void dgvMain_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            if (e.ColumnIndex == zcena.Index)
            {
                ...
                minPrice = (decimal)(dgvMain.DataSource as DataTable).DefaultView[e.RowIndex]["minPrice"];
                maxPrice = (decimal)(dgvMain.DataSource as DataTable).DefaultView[e.RowIndex]["maxPrice"];
                prc = (decimal)(dgvMain.DataSource as DataTable).DefaultView[e.RowIndex]["prnc"];
                if (decimaltest > maxPrice || decimaltest < minPrice)
                {
                    MessageBox.Show(Config.centralText($"Введённая цена выходит за\nдиапазон цены, определяемой\nпроцентом наценки {prc.ToString("0.00")}%\n"), "Проверка цены", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
            }
        }
